...
const Plugs = () => {
    ...
    return (
        <div className="flex flex-col justify-start items-center">
            <div className="relative isolate mt-[40px] lg:mt-[30px] pt-[40px] pb-[60px] lg:pt-[30px] bg-no-repeat bg-cover bg-center flex justify-center h-[70vh] bg-[url('/images/bg.png')]">
                    ...
                    <div className="max-w-2xl flex items-center">
                        ...
                        <div className="text-center">
                            ...
                            <div className="mt-24 flex items-center justify-center gap-x-6">
                                <select name=""  
                                    className="rounded-sm bg-radial-gradient px-12 py-2 text-sm font-extrabold text-black bg-[#F0F0F0] shadow-sm" id="" 
                                ...
                                <select name=""  
                                    className="rounded-sm border-blue border px-12 py-2 text-sm font-semibold text-blue shadow-sm" id="" 
                                    onChange={(e) => setSelectedLocation(e.target.value)}
                                >
                                    <option value="">Location</option>
                                    {locations?.map((locate, idx) =>{
                                        return(
                                            <option key={idx} value={locate}>{locate}</option>
                                        );
                                    })}
                                </select>
                            </div>
                ...
                < div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-16">
                    {displayplugs?.length >= 0 &&
                        displayplugs.map((item, index) => (
                            <SinglePlugCard key={index} index={index} item={item} />
                        ))}
                </div>
            </div >
        </div>
