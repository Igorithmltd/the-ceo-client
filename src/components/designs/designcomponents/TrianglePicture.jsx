...
const TrianglePicture = props => {
  const api = ApiSetup();
  const { x, y, containerStyle, selfStyle, url_type, userData, url, Alt, iwidth, iheight,formEdit, setFormEdit, content, index, setFormOnePicItem, setFormItem, maxLength} = props;
  ...
  return (
    ...
    >
      {( url_type === "logo" && (userData.logo === null || userData.logo === "" )) ? [
        <p key="a1" className={`bg-fixed flex flex-row justify-center items-center font-Montserrat-ExtraBold rounded-lg ${selfStyle}`}
            style={{ fontSize: `${iwidth/1.8}px`,height:`${iheight}px`, width:`${iwidth}px`, 
              color:userData.maincolor, rotate:`${props.degree}deg`, 
                opacity: (uploadProgress > 0) ? 0.3 : `${props.opacity}%`,
                  borderRadius:`${props.tl}px ${props.tr}px ${props.br}px ${props.bl}px`}}>
            {initials}
        </p>
      ] : [
        <DesignImageTrabed key="a" src={processur()} alt={`${Alt}`}
          className={`bg-fixed triangle ${selfStyle}`}
        ...
        <Box className="w-11/12 mt-4">
          <Stack sx={sty} className="flex flex-col">
            <div className='flex flex-col w-full'>
              ...
              <Stack key="b" direction="row" className=" w-full h-30 flex flex-row justify-center">
                {(url_type !== "editable") ? [
                  <div key="a1">
                  ...
                  <div key="a" className='flex flex-row '>
                    <div className='flex flex-col w-full'>
                      <input key='fbd' id='logo' type="file" onChange={(e) => { handleCrop(); setFormEdit(false); setFile(URL.createObjectURL(e.target.files[0])) }} className='w-11/12 p-2 m-2 justify-self-center hidden' accept="image/*" />
