...
const ServiceSales = ({ item, addServiceToCart, selectedCategory }) => {
  ...
  return (
    <div
      key={item.id}
      className="max-w-[548px] max-h-[300px] flex flex-col justify-between p-4 bg-white rounded-lg shadow-lg border border-gray-200"
    >
      <div className="flex">
        <div className="md:max-w-[548px] md:max-h-[300px] flex flex-col w-[33%] gap-4">
        ...
        <div className="flex flex-col justify-between gap-4 p-4 w-2/3">
          <div>
            <h3 className="text-[13px] mdtext-[18px] font-bold mb-1">{item.name}</h3>
      ...
      <div className="flex justify-between gap-4 mt-4 w-[100%] mx-auto">
        <button
          onClick={() => addServiceToCart({ ...item, quantity, type: "service" })}
